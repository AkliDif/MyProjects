...
{
	$donnees = executeRequete("SELECT id_produit,reference,titre, marque, photo,prix FROM produit WHERE categorie='$_GET[categorie]'");
	
	while($produit = $donnees->fetch_assoc())
	{
		$contenu .= '<div class="boutique-produit">';
		$contenu .= "<h3>$produit[titre]</h3>";
		$contenu .= "<h3>$produit[marque]</h3>";
...
else if(isset ($_POST['filtre']))
{
	...
	switch ($typeFiltre) :
		case "Triparmarque" : 
			$donnees = executeRequete("SELECT id_produit,reference,titre, marque, photo,prix FROM produit ORDER BY marque ASC");	
			while($produit = $donnees->fetch_assoc())
			{
				$contenu .= '<div class="boutique-produit">';
				$contenu .= "<h3>$produit[titre]</h3>";
		...
		case "Prixcroissant" :
			$donnees = executeRequete("SELECT id_produit,reference,titre,marque,photo,prix FROM produit ORDER BY prix ASC");	
			while($produit = $donnees->fetch_assoc())
			{
				$contenu .= '<div class="boutique-produit">';
				$contenu .= "<h3>$produit[titre]</h3>";
		...
		case "Prixdecroissant" :
			$donnees = executeRequete("SELECT id_produit,reference,titre, marque,photo,prix FROM produit ORDER BY prix DESC");	
			while($produit = $donnees->fetch_assoc())
			{
				$contenu .= '<div class="boutique-produit">';
				$contenu .= "<h3>$produit[titre]</h3>";
...
{
	$donnees = executeRequete("SELECT id_produit,reference,titre,marque,photo,prix FROM produit");
